    ...
    <div class="col-md-8">
        <div class="card mb-4">
            <div class="card-body">
                ...
                {{-- Tombol Pinjam Buku untuk User Biasa jika stok > 0 --}}
                @if(Auth::check() && Auth::user()->role === 'user' && $book->stok > 0)
                    <form action="{{ route('user.borrowings.store') }}" method="POST" class="d-inline">
                        @csrf
                        <input type="hidden" name="book_id" value="{{ $book->id }}">
                        <button type="submit" class="btn btn-primary" onclick="return confirm('Apakah Anda yakin ingin meminjam buku ini?')">
                            <i class="fas fa-hand-holding-heart me-1"></i> Pinjam Buku Ini
                        </button>
                    </form>
                @endif
